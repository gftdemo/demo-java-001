...
public class Comment {
  ...
  public static List<Comment> fetchAll() { // Renamed to match the regular expression '^[a-z][a-zA-Z0-9]*$'
    ...
    try {
      ...
      try {
        if (stmt != null) stmt.close(); // Closed in finally clause
  ...
  public static boolean delete(String id) { // Use a primitive boolean expression here
    ...
    try {
      ...
      try {
        if (pStatement != null) pStatement.close(); // Closed in finally clause
      } catch (SQLException se) {
        se.printStackTrace();
      }
    }
  ...
  private boolean commit() throws SQLException { // Use a primitive boolean expression here
    PreparedStatement pStatement = null;
    try {
